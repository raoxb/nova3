...
import IlIlllIIlI1.llllIIIIll1;
...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m1718constructorimpl = SegmentOrClosed.m1718constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1718constructorimpl = SegmentOrClosed.m1718constructorimpl(segment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1723isClosedimpl(m1718constructorimpl)) {
                Segment m1721getSegmentimpl = SegmentOrClosed.m1721getSegmentimpl(m1718constructorimpl);
                while (true) {
                    Segment segment4 = (Segment) this.tail;
                    if (segment4.getId() >= m1721getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!m1721getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment4, m1721getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m1721getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1721getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1721getSegmentimpl(m1718constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment4, m1721getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m1721getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
