...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1669trySendJP2dKIU(E element) {
        Object offerInternal = offerInternal(element);
        if (offerInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1689successJP2dKIU(Unit.INSTANCE);
        }
        if (offerInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1688failurePtdJZtk() : ChannelResult.INSTANCE.m1687closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (offerInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1687closedJP2dKIU(helpCloseAndGetSendException((Closed) offerInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (callUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m163constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1714remove()) {
                receive.helpRemove();
            } else {
                m1706constructorimpl$default = InlineList.m1711plusFjFbRPM(m1706constructorimpl$default, receive);
            }
        }
        if (m1706constructorimpl$default != null) {
            if (!(m1706constructorimpl$default instanceof ArrayList)) {
                ((Receive) m1706constructorimpl$default).resumeReceiveClosed(closed);
            } else if (m1706constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) m1706constructorimpl$default;
